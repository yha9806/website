...
test.describe('VULCA 47D Display Issues', () => {
  ...
  test('检查47D按钮和数据显示问题', async ({ page }) => {
    ...
    const hasButton = await button47D.isVisible();
    ...
    if (hasButton) {
      ...
      const isVulcaVisible = await vulcaSection.isVisible();
    ...
    if (await button47D.isVisible()) {
      ...
      for (const mode of viewModes) {
        ...
        if (hasMode) {
          ...
          // 检查切换后的内容变化
          const content = await page.locator('.vulca-visualization').innerHTML();
