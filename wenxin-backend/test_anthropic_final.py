"""
Test available Anthropic models with benchmark tests
"""
import asyncio
import json
import os
...
class AnthropicBenchmark:
    def __init__(self):
        ...
        # Available Anthropic models (confirmed working)
        self.models_to_test = [
    ...
    async def run_all_tests(self):
        ...
        output_file = output_dir / "anthropic_final.json"
...
"""
    ...
    for model_id, results in model_results.items():
        display_name = results[0].get('display_name', model_id)
        md_content += f"\n### {display_name}\n"
    ...
    md_content += "\n## Notes\n\n"
    md_content += "- All models were successfully tested with the provided Anthropic API key\n"
    md_content += "- Scoring was performed using GPT-4o-mini for consistency\n"
    md_content += "- Claude 3.5 Sonnet represents the latest and most capable model\n"
    md_content += "- Claude 3 Opus provides strong reasoning capabilities\n"
    md_content += "- Haiku models are optimized for speed and efficiency\n"
    
    # Save markdown report
    md_file = Path("benchmark_results/anthropic/anthropic_final.md")
