...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % 背景：玻璃拟态 + 体积光
  \path[rounded corners=18pt, top color=black!80!blue!50, bottom color=black!70!cyan!60]
    (0,0) rectangle (\W,\H);
  \path[fill=white, fill opacity=0.04, rounded corners=16pt, draw=white!10]
    (0.7,0.7) rectangle (\W-0.7,\H-0.7);
  \path[fill=cyan!40, opacity=0.08, rounded corners=30pt, rotate=8]
    (5,0) ellipse (8 and 12);
  \path[fill=magenta!35, opacity=0.08, rounded corners=30pt, rotate=-12]
    (25,5) ellipse (9 and 11);

  % 霓虹线框手感（抽象网格）
  \foreach \i in {0,...,10} {
    \draw[cyan!50, opacity=0.3, line width=0.4pt]
      (6+0.8*\i,3) -- (4+0.6*\i,12);
  }
  \foreach \j in {0,...,9} {
    \draw[magenta!50, opacity=0.3, line width=0.4pt]
      (4,3+0.9*\j) .. controls (7,4+0.8*\j) .. (12,2+1.2*\j);
  }
  \path[fill=white, fill opacity=0.12, draw=cyan!40, rounded corners=8pt]
    (6.5,7.5) -- (9.5,9) -- (11.2,7.2) -- (9.4,5.5) -- cycle;
  \path[fill=white, fill opacity=0.10, draw=magenta!40, rounded corners=8pt]
    (7.4,4.2) -- (9.9,5.1) -- (8.8,6.8) -- (6.9,5.7) -- cycle;
  ...
  \node[anchor=west, text=white!85, font=\fontsize{11pt}{13pt}\selectfont] at (1.25,17.5)
    {Surreal hand × digital wireframe × neon holography（Glassmorphism，Volumetric lighting）};
  \node[anchor=east, text=white!80, font=\fontsize{9pt}{11pt}\selectfont] at (\W-1.2,18.4)
    {3:2｜30\,cm×20\,cm｜XeLaTeX + TikZ};

  % 左列：平台概览
  \path[rounded corners=10pt, fill=white, fill opacity=0.07, draw=white!35]
  ...
  \node[anchor=north west, text=white!90, font=\fontsize{10pt}{13pt}\selectfont] at (1.6,15.8) {\begin{minipage}{7.6cm}
    • 42 模型/15 机构，28 真实模型上线；评分、雷达图与对战实时刷新。\\[2pt]
    • 前端 React 19 + Tailwind（iOS＋玻璃拟态），文本高亮与智能切块。\\[2pt]
    • 后端 FastAPI + SQLAlchemy，统一模型接口，WebSocket 战况推送。\\[2pt]
    • 部署：GCP Cloud Run + Cloud SQL + Cloud Storage；CI/CD 15 分钟。\\[2pt]
    • 页面：排行榜 / 模型详情 / 对战 / 评测任务 / 登录；52 组件，64 E2E 用例。
  \end{minipage}};

  % 左列：VR 案例
  \path[rounded corners=8pt, fill=white, fill opacity=0.08, draw=white!30]
    (1.2,2.2) rectangle (9.5,4.8);
  \node[anchor=west, text=cyan!15, font=\bfseries\fontsize{10pt}{12pt}\selectfont] at (1.5,4.5)
    {VR/AR 案例｜触摸线框 → 虚实切换};
  \node[anchor=west, text=white!88, font=\fontsize{9pt}{12pt}\selectfont] at (1.5,3.9)
    {提示词：Surreal blend of hand + wireframe mesh，Bright neon colors，Holography};
  \node[anchor=west, text=white!70, font=\fontsize{8pt}{10pt}\selectfont] at (1.5,3.1)
    {可加：Glassmorphism, Volumetric lighting, Bioluminescence，Rule of thirds 构图};
  ...
  \node[anchor=north west, text=white, font=\bfseries\fontsize{14pt}{16pt}\selectfont] at (10.6,16.4)
    {端到端架构};
  \node[rounded corners=6pt, fill=cyan!40, draw=white!70, text=black, font=\bfseries] (user) at (13,14.8) {用户 / 游客};
  \node[rounded corners=6pt, fill=white, fill opacity=0.92, text=black, font=\bfseries] (front) at (13,12.8)
    {前端 React19 + Tailwind};
  \node[rounded corners=6pt, fill=white, fill opacity=0.88, text=black, font=\bfseries] (backend) at (13,10.8)
    {FastAPI 服务层};
  \node[rounded corners=6pt, fill=white, fill opacity=0.88, text=black, font=\bfseries] (umi) at (13,8.8)
    {统一模型接口 + 评测引擎};
  \node[rounded corners=6pt, fill=white, fill opacity=0.88, text=black, font=\bfseries] (providers) at (13,6.8)
    {8 家模型 Provider};
  ...
  \node[rounded corners=6pt, fill=white, fill opacity=0.07, draw=white!25, text=white, font=\fontsize{9pt}{12pt}\selectfont, align=left] at (16.8,9.8) {多阶段评测\\• 分析提示词\\• 生成/润色\\• 质量评估\\• 结果可视化};
  \node[rounded corners=6pt, fill=white, fill opacity=0.07, draw=white!25, text=white, font=\fontsize{9pt}{12pt}\selectfont, align=left] at (16.8,6.4) {数据层\\PostgreSQL / Redis / 对象存储\\评分缓存 + 指标索引};
  ...
  \foreach [count=\i] \name/\val in {gpt-4o-mini/87.2,gpt-4o/86.7,o1/86.5,gpt-4-turbo/86.0,gpt-4/86.0} {
    ...
    \path[fill=cyan!55, draw=white!20, rounded corners=2pt]
  ...
  \draw[white!30] (\barX,\barY-5.7) -- ++(\barMax,0);
  \node[anchor=west, text=white!60, font=\fontsize{8pt}{10pt}\selectfont] at (\barX,\barY-6.3) {Glassmorphism + Volumetric lighting 视觉基调};
  ...
  \path[fill=cyan!50, draw=cyan!80, opacity=0.55] (Q1) -- (Q2) -- (Q3) -- cycle;
  \path ( \cx,\cy ) ++(90:\r*0.78) coordinate (L1);
  \path ( \cx,\cy ) ++(210:\r*0.71) coordinate (L2);
  \path ( \cx,\cy ) ++(330:\r*0.74) coordinate (L3);
  \path[fill=yellow!40, draw=yellow!80, opacity=0.45] (L1) -- (L2) -- (L3) -- cycle;
  ...
  \node[anchor=west, text=white!60, font=\fontsize{8pt}{10pt}\selectfont] at (26.8,7.6)
    {色调：霓虹 + 全息；光影：Volumetric + Refraction};

  % 底部 CTA
  \path[rounded corners=10pt, fill=cyan!60, draw=white!70] (1.2,1.0) rectangle (28.8,1.8);
