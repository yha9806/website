...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  % 背景与网格感
  \path[rounded corners=18pt, top color=black!90!blue!60, bottom color=black!70!cyan!40]
    (0,0) rectangle (\W,\H);
  \path[rounded corners=14pt, draw=white!8, fill=white, fill opacity=0.03]
    (0.7,0.7) rectangle (\W-0.7,\H-0.7);
  \foreach \x in {1,...,14} {
    \draw[white!6] (0.7+2*\x,0.7) -- (0.7+2*\x,\H-0.7);
  }
  \foreach \y in {1,...,9} {
    \draw[white!6] (0.7,0.7+2*\y) -- (\W-0.7,0.7+2*\y);
  }
  ...
  \node[anchor=west, text=white!85, font=\fontsize{11pt}{13pt}\selectfont] at (1.25,17.5)
    {VULCA文化理解框架（来自 2025.findings-emnlp.103.pdf） × 生产级多模型评测与对战};
  \node[anchor=east, text=white!80, font=\fontsize{9pt}{11pt}\selectfont] at (\W-1.2,18.4)
    {3:2 比例｜30\,cm × 20\,cm｜适配 XeLaTeX + TikZ};

  % 左列：平台概览
  \path[rounded corners=10pt, fill=white, fill opacity=0.08, draw=white!40]
  ...
  \node[anchor=north west, text=white!90, font=\fontsize{10pt}{13pt}\selectfont] at (1.6,15.8) {\begin{minipage}{7.6cm}
    • 42 个 AI 模型，15 家机构；28 个真实模型已上线，评分与雷达图可视化。\\[2pt]
    • 前端：React 19 + TypeScript + Tailwind（iOS 质感），集成文本高亮与智能切块。\\[2pt]
    • 后端：FastAPI + SQLAlchemy，统一模型接口 + WebSocket 战况推送。\\[2pt]
    • 真实部署：GCP Cloud Run + Cloud SQL + Cloud Storage，CI/CD 15m 自动化。\\[2pt]
    • 主要页面：排行榜 / 模型详情 / 对战 / 评测任务 / 登录；52 组件、64 条 E2E 用例。
  \end{minipage}};

  % 左列：案例
  \path[rounded corners=8pt, fill=white, fill opacity=0.08, draw=white!30]
    (1.2,2.2) rectangle (9.5,4.8);
  \node[anchor=west, text=cyan!20, font=\bfseries\fontsize{10pt}{12pt}\selectfont] at (1.5,4.5) {案例｜《十二月令》多视角批注};
  \node[anchor=west, text=white!88, font=\fontsize{9pt}{12pt}\selectfont] at (1.5,3.9)
    {游客模式一键体验 → Persona 引导提示 → 文本高亮/切块展示 → 对战投票实时刷新};
  \node[anchor=west, text=white!70, font=\fontsize{8pt}{10pt}\selectfont] at (1.5,3.1)
    {亮点：文化语境提示、评分提示框、缓存版本控制 v2.1.0、HashRouter 路由修复};
  ...
  \node[rounded corners=6pt, fill=cyan!30, draw=white!60, text=black, font=\bfseries] (user) at (13,14.8) {用户 / 游客};
  \node[rounded corners=6pt, fill=white, fill opacity=0.9, text=black, font=\bfseries] (front) at (13,12.8)
    {前端 React19 + Tailwind};
  \node[rounded corners=6pt, fill=white, fill opacity=0.85, text=black, font=\bfseries] (backend) at (13,10.8)
    {FastAPI 服务层};
  \node[rounded corners=6pt, fill=white, fill opacity=0.85, text=black, font=\bfseries] (umi) at (13,8.8)
    {统一模型接口 + 评测引擎};
  \node[rounded corners=6pt, fill=white, fill opacity=0.85, text=black, font=\bfseries] (providers) at (13,6.8)
    {8 家模型 Provider};
  ...
  \node[rounded corners=6pt, fill=white, fill opacity=0.08, draw=white!30, text=white, font=\fontsize{9pt}{12pt}\selectfont, align=left] at (16.8,9.8) {多阶段评测\\• 分析提示词\\• 生成/润色\\• 质量评估\\• 结果可视化};
  \node[rounded corners=6pt, fill=white, fill opacity=0.08, draw=white!30, text=white, font=\fontsize{9pt}{12pt}\selectfont, align=left] at (16.8,6.4) {数据层\\PostgreSQL / Redis / 对象存储\\评分缓存 + 指标索引};
  ...
  \node[anchor=west, text=white!60, font=\fontsize{8pt}{10pt}\selectfont] at (\barX,\barY-6.3) {评分：GPT-4o-mini 领先，稳定优秀段};
  ...
  \node[anchor=west, text=white!60, font=\fontsize{8pt}{10pt}\selectfont] at (26.8,7.6)
    {数据来源：VULCA EMNLP 2025 框架（pdf 第1页摘要）};

  % 评测流程卡片
  \path[rounded corners=8pt, fill=white, fill opacity=0.08, draw=white!35]
    (20.4,2.2) rectangle (28.8,4.8);
  \node[anchor=west, text=cyan!20, font=\bfseries\fontsize{10pt}{12pt}\selectfont] at (20.7,4.5) {平台评测流程};
  \node[anchor=west, text=white!88, font=\fontsize{9pt}{12pt}\selectfont] at (20.7,3.9)
    {创建评测任务 → AI 生成/批注 → 文本高亮/切块 → 雷达图与条形图 → 对战投票};
  \node[anchor=west, text=white!70, font=\fontsize{8pt}{10pt}\selectfont] at (20.7,3.1)
    {支撑：Zustand 状态流、Axios 拦截、缓存版本校验、64 条 E2E 测试};
  ...
  \node[anchor=west, text=black, font=\bfseries\fontsize{10pt}{12pt}\selectfont] at (1.6,1.3)
    {立即体验：前端 http://localhost:5173 ｜ 后端 http://localhost:8001 ｜ 演示账号 demo/demo123};
  \node[anchor=east, text=black, font=\fontsize{9pt}{11pt}\selectfont] at (28.4,1.3)
    {更多：VULCA 论文摘要、openai\_benchmark\_v2\_report.md、PROJECT\_STATUS\_REPORT.md};
\end{tikzpicture}
\end{document}
